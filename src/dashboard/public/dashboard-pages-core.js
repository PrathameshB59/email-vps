(function dashboardPagesBoot() {
  ...
  async function loadSecurityPage() {
    ...
    if (summaryMount) {
      summaryMount.innerHTML = [
        ...
        renderSummaryCard({
          label: "Fail2Ban",
          value: security.controls.fail2banAvailable ? "Active" : "Unavailable",
          sub: security.controls.fail2banJailSummary || "Host control visibility",
          tone: security.controls.fail2banAvailable ? "secure" : "warning",
        }),
        renderSummaryCard({
          label: "AIDE Baseline",
          value: security.controls.aideBaselinePresent ? "Present" : "Missing",
          sub: security.controls.aideLastCheckAt
            ? `Last check ${formatTime(security.controls.aideLastCheckAt)}`
            : fmt(security.controls.lastDailyReportPath),
      ...
      controlsMount.innerHTML = [
        renderPanel({
          ...
          bodyHtml: renderKeyValueList([
            ...
            {
              label: "Control freshness",
              value:
                security.controls.controlFreshness == null
                  ? "-"
                  : `${fmt(security.controls.controlFreshness)} minutes`,
            },
          ]),
        }),
        renderPanel({
          ...
          bodyHtml: renderKeyValueList([
            ...
            { label: "Fail2Ban summary", value: security.controls.fail2banJailSummary || "-" },
            {
              label: "Fail2Ban jails",
              value:
                Array.isArray(security.controls.fail2banJails) && security.controls.fail2banJails.length
                  ? security.controls.fail2banJails.join(", ")
                  : "-",
            },
            { label: "AIDE last check", value: formatTime(security.controls.aideLastCheckAt) },
          ]),
        }),
      ].join("");
    }
  ...
  async function loadProgramsPage() {
    ...
    if (summaryMount) {
      summaryMount.innerHTML = [
        ...
        renderSummaryCard({
          ...
          value: fmt(programs.cronSchedulerStatus?.state || "unknown"),
          sub: programs.cronMetricsJobStatus?.message || "Metrics cron diagnostics",
          tone: programs.cronSchedulerStatus?.health || "unknown",
        }),
      ].join("");
    }
    ...
    if (infraMount) {
      ...
      infraMount.innerHTML = [
        renderPanel({
          ...
          bodyHtml:
            ...
            renderKeyValueList([
              ...
              { label: "cron scheduler", value: fmt(programs.cronSchedulerStatus?.state || "-") },
              {
                label: "cron metrics job",
                value: fmt(programs.cronMetricsJobStatus?.message || programs.cronMetricsJobStatus?.health || "-"),
              },
              {
                label: "postfix warning count",
                value: fmt(Array.isArray(programs.postfixConfigWarnings) ? programs.postfixConfigWarnings.length : 0),
              },
            ]),
        }),
        renderPanel({
        ...
        renderPanel({
          title: "Postfix Config Warnings",
          meta: "Duplicate and overridden key warnings detected from main.cf parsing.",
          bodyHtml: renderSimpleTable({
            columns: [
              { key: "key", label: "Key", render: (row) => row.key },
              { key: "severity", label: "Severity", render: (row) => row.severity },
              { key: "lineList", label: "Lines", render: (row) => row.lineList },
              { key: "message", label: "Message", render: (row) => row.message },
            ],
            rows: postfixWarningRows,
            emptyMessage: "No postfix config warnings detected.",
          }),
        }),
      ].join("");
    }
  ...
  async function loadMailPage() {
    ...
    let otpDiagnostics = null;
    try {
      otpDiagnostics = await fetchJson("/api/v1/dashboard/otp-delivery?limit=15");
    } catch (error) {
      otpDiagnostics = null;
    }
    ...
    setSubline(
      `Last refresh ${formatTime(new Date().toISOString())} | Relay ${
        mailCheck.relay.ok ? "healthy" : "degraded"
      } | Postfix warnings ${fmt(mailCheck.postfixWarningCounts?.total || 0)} | Probe cooldown ${fmt(
        mailCheck.probe.remainingCooldownSeconds
      )}s${
        otpDiagnostics ? ` | OTP delivery events ${fmt(otpDiagnostics.events?.length || 0)}` : ""
      }`
    );
    ...
    if (diagnosticsMount) {
      ...
      const postfixConfigIssuesRows = (mailCheck.postfixConfigHealth?.issues || []).map((issue) => ({
        key: escapeHtml(fmt(issue.key)),
        severity: `<span class="${badgeClass(issue.severity)}">${escapeHtml(fmt(issue.severity))}</span>`,
      ...
      diagnosticsMount.innerHTML = [
        renderPanel({
          ...
          bodyHtml: renderKeyValueList([
            ...
            { label: "Postfix config health", value: fmt(mailCheck.postfixConfigHealth?.health || "-") },
            { label: "Cron noise health", value: fmt(mailCheck.cronNoiseHealth?.health || "-") },
            {
              label: "Logwatch warnings",
              value: fmt(mailCheck.logwatchSummary?.warningCount || 0),
            },
          ]),
        }),
        renderPanel({
        ...
        renderPanel({
          title: "Cron, Logwatch, and Postfix Warnings",
          meta: "Operational warning signatures from postfix config, cron runtime, and logwatch parsing.",
          bodyHtml:
            renderKeyValueList([
              { label: "Cron scheduler state", value: fmt(mailCheck.cronNoiseHealth?.schedulerState || "-") },
              {
                label: "Cron stale references",
                value: fmt(mailCheck.cronNoiseHealth?.staleReferences || 0),
              },
              {
                label: "Cron expected references",
                value: fmt(mailCheck.cronNoiseHealth?.expectedReferences || 0),
              },
              {
                label: "Logwatch source",
                value: fmt(mailCheck.logwatchSummary?.source || "-"),
              },
              {
                label: "Postfix warning total",
                value: fmt(mailCheck.postfixWarningCounts?.total || 0),
              },
            ]) +
            renderSimpleTable({
              columns: [
              ...
              rows: postfixConfigIssuesRows,
              emptyMessage: "No postfix config issues in current diagnostics snapshot.",
            }),
        }),
      ].join("");
    }
    ...
    const otpDeliveryMount = document.getElementById("mailOtpDelivery");
    if (otpDeliveryMount) {
      if (!otpDiagnostics) {
        otpDeliveryMount.innerHTML = renderPanel({
          title: "OTP Delivery Diagnostics",
          meta: "Delivery event telemetry for OTP sign-in emails.",
          bodyHtml:
            '<p class="meta">OTP diagnostics endpoint is disabled or unavailable in current environment.</p>',
        });
      } else {
        const rows = (otpDiagnostics.events || []).map((event) => ({
          time: escapeHtml(formatTime(event.createdAt)),
          request: escapeHtml(fmt(event.otpRequestId)),
          stage: `<span class="${badgeClass(event.deliveryStage)}">${escapeHtml(
            fmt(event.deliveryStage)
          )}</span>`,
          recipient: escapeHtml(fmt(event.recipientEmail)),
          error: escapeHtml(fmt(event.errorCode || event.errorMessage || "-")),
        }));

        const summary = Array.isArray(otpDiagnostics.failureSummary)
          ? otpDiagnostics.failureSummary
              .map((item) => `${fmt(item.key)}: ${fmt(item.count)}`)
              .join(" | ")
          : "-";

        otpDeliveryMount.innerHTML = renderPanel({
          title: "OTP Delivery Diagnostics",
          meta: "Trace OTP request IDs, relay acceptance, and recent delivery failures.",
          bodyHtml:
            renderKeyValueList([
              { label: "Events loaded", value: fmt(otpDiagnostics.events?.length || 0) },
              { label: "Failure summary", value: summary || "-" },
            ]) +
            renderSimpleTable({
              columns: [
                { key: "time", label: "Time", render: (row) => row.time },
                { key: "request", label: "Request ID", render: (row) => row.request },
                { key: "stage", label: "Stage", render: (row) => row.stage },
                { key: "recipient", label: "Recipient", render: (row) => row.recipient },
                { key: "error", label: "Error", render: (row) => row.error },
              ],
              rows,
              emptyMessage: "No OTP delivery events captured yet.",
            }),
        });
      }
    }
  ...
  async function loadOperationsPage() {
    ...
    setSubline(
      ...
      } | Open issues ${fmt(operations.totals?.open || 0)} | Freshness ${fmt(
        operations.freshnessSeconds
      )}s${activeFilters.length ? ` | Filters ${activeFilters.join(", ")}` : ""}`
    );
      ...
      controlsMount.innerHTML = [
        renderPanel({
          ...
          bodyHtml: renderKeyValueList([
            ...
            { label: "Relay", value: relay.ok ? "healthy" : `degraded (${fmt(relay.errorCode || "-")})` },
        ...
        renderPanel({
          ...
          bodyHtml: renderKeyValueList([
            ...
            {
              label: "Postfix warning total",
              value: fmt(operations.mailRuntime?.postfixWarningCounts?.total || 0),
            },
            {
              label: "Cron stale references",
              value: fmt(operations.mailRuntime?.cronNoiseHealth?.staleReferences || 0),
            },
            {
              label: "Logwatch warning count",
              value: fmt(operations.mailRuntime?.logwatchSummary?.warningCount || 0),
            },
          ]),
        }),
      ].join("");
    ...
    if (hintsMount) {
      ...
      hintsMount.innerHTML = renderPanel({
        ...
        bodyHtml: `<div class="ops-hints">${hints
          .map((cmd) => `<code class="mono">${escapeHtml(cmd)}</code>`)
          .join("")}</div>`,
      });
    }
  }
